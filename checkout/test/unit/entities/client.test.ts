...
test.each(["406.302.170-37", "406.302.170", "406"])(
  "not should created new Client, because invalid CPF",
  function (cpf: string) {
    expect(() => new Client("", cpf, "", "")).toThrow("Invalid cpf");
  }
);

test.each(["407.302.170-27", "684.053.160-00", "746.971.314-01"])(
  "should created new Client validated %s",
  function (cpf: string) {
    expect(() => new Client("", cpf, "", "")).toBeDefined();
  }
);
