...
test("must create an order with email", async function () {
  const productsRepositoryStub = sinon
    .stub(ProductInMemoryRepository.prototype, "get")
    .resolves(new Product("1", "Dove", "shampoo", 9.0));
  let products = [new Product("1", "Dove", "shampoo", 17.0)];
...
test("must not repeat item in input", async function () {
  ...
  expect(() => checkout.execute(input)).rejects.toThrow(
    new Error("must not repeat item")
  );
});
